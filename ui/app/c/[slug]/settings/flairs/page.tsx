...
export default function FlairsPage({ params }: any) {
  ...
  const [form, setForm] = useState<FlairForm>({
    name: '',
    textColor: '#000000',
    bgColor: '#eeeeee',
    isUserFlair: false,
  ...
  async function del(id: string) {
    const r = await fetch(`${API}/api/communities/${slug}/flairs/${id}/delete`, {
      method: 'POST',
      ...
      <form onSubmit={create} className="space-y-2 border p-3 rounded" aria-label="create flair">
        <div className="flex gap-2 items-center">
          ...
          <label className="flex items-center gap-1">
            <input
              type="checkbox"
              checked={form.isUserFlair}
              onChange={e => setForm(prev => ({ ...prev, isUserFlair: e.target.checked }))}
            />
            {t('User flair')}
          </label>
          <label className="flex items-center gap-1">
            <input
              type="checkbox"
              checked={form.modOnly}
              onChange={e => setForm(prev => ({ ...prev, modOnly: e.target.checked }))}
            />
            {t('Mod-only')}
          </label>
          <button className="btn" type="submit" aria-label="create flair">
            {t('Create')}
          </button>
        </div>
      </form>
        ...
        {rows.map((f: any) => (
          <li key={f.id} className="border p-2 rounded flex justify-between" role="listitem">
            <span
              ...
              aria-label={`flair ${f.name}`}
            >
              {f.name}
            </span>
            <button className="btn" onClick={() => del(f.id)} aria-label={`delete flair ${f.name}`}>
              {t('Delete')}
            </button>
          </li>
        ))}
